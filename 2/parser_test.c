...
static void test_two_words(void) {
  ...
  unit_check(e->cmd.arg_count == 1, "arg count");
  unit_check(strcmp(e->cmd.args[0], "my file with whitespaces in name.txt") ==
                 0,
             "arg[0]");
  unit_check(e->next == NULL, "no more exprs");
  command_line_delete(line);
...
static void test_escape_in_string(void) {
  ...
  unit_check(e->cmd.arg_count == 1, "arg count");
  unit_check(strcmp(e->cmd.args[0], "123 >&| 456 \\\" str \\\"") == 0,
             "arg[0]");
  unit_check(e->next == NULL, "no more exprs");
  command_line_delete(line);
  ...
  unit_check(e->cmd.arg_count == 1, "arg count");
  unit_check(strcmp(e->cmd.args[0], "import time\\n"
                                    "time.sleep(0.1)\\n"
                                    "f = open('test.txt', 'a')\\n"
                                    "f.write('Text\\\\n')\\n"
                                    "f.close()\\n") == 0,
...
static void test_output_redirect(void) {
  ...
  unit_check(parser_pop_next(p, &line) == PARSER_ERR_NONE, "parse");
  unit_check(line->out_type == OUTPUT_TYPE_FILE_NEW, "out type");
  unit_check(strcmp(line->out_file, "my file with whitespaces in name.txt") ==
                 0,
             "out file");
  ...
  unit_check(strcmp(e->cmd.exe, "echo") == 0, "exe");
  unit_check(e->cmd.arg_count == 1, "arg count");
  unit_check(strcmp(e->cmd.args[0], "123 456 \\\" str \\\"") == 0, "arg[0]");
  unit_check(e->next == NULL, "no more exprs");
  command_line_delete(line);
...
static void test_pipe(void) {
  ...
  str = "echo 'source string' | sed 's/source/destination/g' | sed "
        "'s/string/value/g'";
  len = strlen(str);
  for (uint32_t i = 0; i < len; ++i) {
    parser_feed(p, &str[i], 1);
  ...
  unit_check(e->cmd.arg_count == 1, "arg count");
  unit_check(strcmp(e->cmd.args[0], "s/source/destination/g") == 0, "arg[0]");
...
static void test_error_one(struct parser *p, const char *expr,
